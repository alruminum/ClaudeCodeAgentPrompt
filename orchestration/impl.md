...
|---|---|---|---|
| `fast` | engineer → commit → pr-reviewer → merge (LLM 2회, 테스트·보안 스킵) | impl에 `(MANUAL)` 태그만 있을 때 / 변수명·설정값 등 단순 변경 | pr_reviewer_lgtm |
| `std` | engineer → commit → test-engineer → vitest → validator → pr-reviewer → merge (LLM 3회) | 일반 구현 (기본값) | pr_reviewer_lgtm |
| `deep` | engineer → commit → test-engineer → vitest → validator → pr-reviewer → security-reviewer → merge (LLM 5회) | impl에 `(BROWSER:DOM)` 태그 있을 때, 또는 보안·품질 게이트 필요 시 | pr_reviewer_lgtm + security_review_passed |
...
flowchart TD
    ...
    subgraph ATTEMPT_LOOP["attempt loop (MAX 3회)"]
        ...
        COMMIT_EARLY["git commit\n(engineer 직후, feature branch)"]
    ...
    COMMIT_EARLY --> SRC_CHK

    SRC_CHK -->|"YES (std/deep)"| TE
    SRC_CHK -->|"NO or fast"| PR_REV
    ...
    PR_RESULT -->|"LGTM (std/fast)"| MERGE
    PR_RESULT -->|"LGTM (deep)"| SEC_REV
    ...
    FAIL_ROUTE["FAIL → 추가 커밋 후 attempt++"]
...
| `@MODE:PR_REVIEWER:REVIEW` | pr-reviewer | validator PASS 후 코드 품질 리뷰 (fast/std/deep 모두) |
...
| `PASS` | validator | pr-reviewer (fast/std/deep 모두) |
| `FAIL` | validator | engineer 추가 커밋 후 재시도 |
| `SPEC_MISSING` | validator | architect MODULE_PLAN (impl 복구) |
| `LGTM` | pr-reviewer | merge (std/fast) 또는 security-reviewer (deep) |
| `CHANGES_REQUESTED` | pr-reviewer | engineer 추가 커밋 후 재시도 |
