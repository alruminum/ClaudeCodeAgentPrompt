...
```
진입 시 체크:
  1. plan_validation_passed 플래그? → architect + validator 스킵 → impl2 (engineer 루프)
...
```
진입 시 역순 체크:
  1. impl 파일 존재? → QA + architect 스킵 → engineer 직접 (impl2 경로)
...
진입: bug 레이블 이슈 OR 유저 버그 직접 보고
      │
      ↓
  [재진입 감지] ─── impl 있음 ──→ engineer 직접 (impl2)
      │               │
