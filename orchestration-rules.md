...
- **Agent 툴 경로 폴백**: `/ux` 등 하네스 루프 없는 스킬은 Agent 툴로 서브에이전트를 호출하는데, in-process라 env var 주입 불가. 이 경로에서는 `agent-gate.py`가 PreToolUse에 쓰는 `{prefix}_{agent}_active` 플래그 파일을 `issue-gate.py`/`commit-gate.py`가 env var 없을 때 폴백으로 확인한다.
- 플래그 cleanup 3단 방어:
...
- **중요**: isolation은 **WorktreeManager가 cwd 전달**로 구현. `claude` CLI에는 `--isolation` 플래그가 존재하지 않으므로 `agent_call`에서 CLI 옵션으로 전달하지 않는다. 전달 시 `error: unknown option '--isolation'`으로 에이전트가 즉시 exit=1로 실패한다.
