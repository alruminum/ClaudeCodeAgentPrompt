...
- **금지 팔레트**: 다크 네이비/미드나잇 배경 + 골드/오렌지/앰버 단일 엑센트 = "Claude/AI 클리셰". 슬립·명상·오디오 류 앱에서 자동 발현되는 판박이 톤. 즉시 다른 색조로 재선정 (jajang 2026-04-26 reject 사례). 인디고/바이올렛 그라디언트도 동일 사유로 금지(기존 룰).
- **라이트/다크 모드 둘 다 정의 필수**: 한 모드만 정의 금지. `## 0. 디자인 가이드` 섹션 하위 `### 컬러 팔레트` 표에 토큰별 Day/Night 두 hex 모두 명시. 둘 다 Anti-AI-Smell 룰 적용. 두 모드는 무드/브랜드 정체성 일관(한 가족), 대비 WCAG AA(4.5:1) 이상, 다크가 단순 반전 아닌 톤다운.
- 상세는 `agents/ux-architect.md` Anti-AI-Smell 섹션 + Step 5 라이트/다크 섹션 참조.
