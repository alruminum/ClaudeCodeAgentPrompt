...
| 버그 보고 | → **qa 스킬** → QA 에이전트 직접 분류 + 라우팅. 상세: [orchestration/impl.md](orchestration/impl.md) (QA/DESIGN_HANDOFF 진입 흐름 섹션)<br>FUNCTIONAL_BUG → `executor.sh impl --issue <N>` / DESIGN_ISSUE → ux 스킬 / SCOPE_ESCALATE → 유저 보고 |
...
→ 상세 (simple): [orchestration/impl_simple.md](orchestration/impl_simple.md)
...
단, `--depth=simple` 플래그로 루프 깊이를 줄이는 것은 허용된다. → depth 상세: [orchestration/impl.md](orchestration/impl.md)
