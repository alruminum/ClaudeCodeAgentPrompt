...
- T3 ralph 교차오염 진단 → `.claude/harness-state/.logs/ralph-cross-session.jsonl` 박제(`ralph-session-stop.py`).
- T4 `_pid-<pid>-<ts>` 폴백 슬롯 청소 → 활성 PID 보존, 죽은 PID 즉시 제거 (`session_state.cleanup_stale_sessions`).
- T1, T2(오피셜 ralph-loop stop-hook claim 가로채기 차단)는 인지된 한계로 잔존 — 현재는 cross-session JSONL 로그로 관측만. 향후 (a) 세션 스코프 symlink / (b) `disabledHooks` 옵션 재평가.
...
### Lint/Build 게이트
automated_checks에 lint/build 실행. `config.lint_command` / `config.build_command` 미설정 시 스킵. 실행 순서: lint → build → commit → test. pre-existing 에러와 구분하기 위해 engineer 수정 파일 스코프로 축소하는 옵션 제공.
