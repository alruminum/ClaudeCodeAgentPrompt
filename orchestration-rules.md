...
- **신규 프로젝트 기본 활성**: `setup-harness.sh`가 `harness.config.json`을 생성할 때 `"isolation": "worktree"`를 기본 포함한다 (2026-04-27부터). 기존 프로젝트는 자동 추가하지 않고 안내만 — 유저가 의도적으로 비활성한 경우 존중.
- **`.gitignore` 자동 등록**: setup 시 `.worktrees/` 항목이 없으면 자동 추가 (git repo일 때만).
...
### ux-architect 디자인 가이드 작성 룰 (Step 5)

UX_FLOW Step 5 "디자인 가이드" 작성 시 강제 규칙:

**1. 구조 패턴 금지 (색 swap 우회 차단)** — 색만 바꾸면 통과되는 룰이 아님. 아래 **구조 조합** 5가지 중 **3개 이상** 만족 시 자동 reject:
- (a) 단색 다크 배경 (네이비/그레이/블랙 색조 무관)
- (b) 단일 채도 엑센트 1색 (골드든 민트든 코랄이든 동일)
- (c) 얇은 outline + 둥근 모서리 + 다크 표면 카드/시트
- (d) 작은 플랫 단색 글리프 아이콘
- (e) "Spotify/Apple Music/Calm 다크모드" 같은 인상

근거: jajang 2026-04-26 reject 사례 1차(골드) → reject 2차(민트). 색 swap만으로 통과되는 룰이라 같은 클리셰 반복. 이번엔 구조 패턴으로 차단.

**2. 카테고리 클리셰 회피 자기 정당화 블록 필수** — `## 0. 디자인 가이드` 최상단에 경쟁 앱 3개 + 그들의 공통 패턴 + 우리의 구조적 차별점(색 변경 아님) + 5가지 자가 점검 명시. 블록 없으면 가이드 미완성 처리.

**3. 라이트 우선 사고** — "밤에 쓰니까 다크" 사고가 클리셰의 출발점. 라이트를 **베이스 정체성**으로 먼저 잡고 다크는 변주. 슬립/명상/오디오 류는 라이트 베이스 + 텍스처/일러스트 정체성 권장 (다크 단색 + 단일 엑센트는 자동 클리셰).

**4. 라이트/다크 모드 둘 다 정의 필수** — 한 모드만 금지. `### 컬러 팔레트` 표에 토큰별 Day/Night 두 hex 모두 명시. 둘 다 Anti-AI-Smell 적용. 무드 일관(한 가족), 대비 WCAG AA(4.5:1) 이상, 다크는 단순 반전 아닌 톤다운.

**5. 표현 매체 1개 이상 채택** — 일러스트 / 사진 / 텍스처 / 그라디언트 / 손글씨 / 픽토그램 / 3D / 콜라주 등 중 정체성 매체 명시. 플랫 미니멀 단독은 클리셰 함정.

상세 + 자가 정당화 블록 형식: `agents/ux-architect.md` Anti-AI-Smell 섹션 참조.
