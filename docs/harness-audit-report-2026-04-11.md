...
> 작성일: 2026-04-11
> 최종 수정: 2026-04-14 — Python 전환 완료 반영 + 로드맵 현행화
...
**총평 스코어: 76.8/100** (Python 전환 완료 반영. 에이전트 팀 전환 시 80+ 예상)
...
| 카테고리 | 초기(4/11) | 현재(4/14) | 가중치 | 가중 점수 | 변경 이유 |
|----------|-----------|-----------|--------|-----------|----------|
| **독창성 (Originality)** | 85 | 85 | 20% | 17.0 | 변동 없음 |
| **아키텍처 품질 (Architecture)** | 78 | **85** | 20% | **17.0** | Bash→Python 전환: 타입 안전, 에러 핸들링, 크로스플랫폼 해결 |
| **실용성 (Practicality)** | 80 | 80 | 15% | 12.0 | 변동 없음 |
| **확장성 (Extensibility)** | 60 | **68** | 15% | **10.2** | config.py에 test_command/lint_command 필드 존재 (연결 대기) |
| **관측 가능성 (Observability)** | 82 | 82 | 10% | 8.2 | 변동 없음 |
| **안전성 (Safety/Guardrails)** | 75 | **77** | 10% | **7.7** | 코드 리뷰 4건 수정 (SIGTERM 핸들러, watchdog, log_fh 누수, 데드코드) |
| **DX/채택 용이성 (Developer Experience)** | 40 | 40 | 10% | 4.0 | 변동 없음 (배포 준비 전) |
| **합계** | **72.3** | | 100% | **76.1** | +3.8p |
...
| # | 항목 | 노력 | 현재 상태 | 다음 액션 |
|---|------|------|----------|----------|
| 1 | test/lint command 설정화 | 소 | config.py 필드 존재, impl_loop.py 미연결 | vitest 하드코딩 → config.test_command 교체 |
| 2 | REFLECTION.md (성공 학습) | 소 | append_success() 있으나 패턴 미추출 | HARNESS_DONE 후 성공 패턴 자동 추출 + reflection 파일 |
| 3 | 도메인별 토큰 예산 | 소 | token_budget: int = 0 (dead field) | dict 구조 + 도메인 감지 + 85% 경고 |
| 4 | Git Worktree 격리 | 중 | 미착수 | Agent Team Phase 1과 연계 |
| 5 | Circuit Breaker (시간 윈도우) | 소 | 미착수 (harness_framework에서 채택) | 동일 fail_type 120초 내 2회 반복 → 조기 에스컬레이션 |
| 6 | "MVP 제외 사항" 강제 | 소 | 미착수 (harness_framework에서 채택) | product-planner + impl 파일에 "구현하지 않는 것" 섹션 필수화 |

### P1 — 아키텍처 진화

| # | 항목 | 노력 | 비고 |
|---|------|------|------|
| 7 | Agent Team Phase 1 (engineer↔test-engineer) | 중 | 컨텍스트 병목(W2) 해소, 방향 검증 |
| 8 | Agent Team Phase 2 (SPEC_GAP 팀화) | 중 | Phase 1 검증 후 |
| 9 | Agent Team Phase 3 (전면 팀 전환) | 대 | depth→팀크기 매핑, ~30% 추가 코드 감소 |
| 10 | BATS → pytest 전환 | 소 | pre-commit hook 호환 복구 |

### P2 — 배포 준비

| # | 항목 | 노력 | 비고 |
|---|------|------|------|
| 11 | 영문 README + quickstart | 중 | 채택 장벽 제거 |
| 12 | 단일 설치 명령 (pip/npm) | 중 | DX 40→60+ |
| 13 | 데모 영상 (3분) | 소 | GitHub issue → loop → merge |
| 14 | 벤치마크 (SWE-bench lite) | 대 | 신뢰도, 학술 레퍼런스 |

### P3 — 차별화 + 커뮤니티

| # | 항목 | 노력 | 비고 |
|---|------|------|------|
| 15 | Docker sandbox 모드 | 중 | 안전성(W3) 해결 |
| 16 | Web UI (trajectory viewer + cost dashboard) | 대 | 가시성, 마케팅 |
| 17 | 3-Tier 확장 (worktree 병렬 + 클라우드 비동기) | 대 | Osmani 프레임워크 적용 |
| 18 | 논문 ("Harness Engineering: Deterministic Multi-Agent Orchestration") | 대 | 명성, 인용 |
| 19 | Plugin system (custom agent, depth, gate) | 대 | 커뮤니티 확장 |
| 20 | harness-memory → vector DB (자동 경험 축적) | 대 | 핵심 차별화 |
...
P0 완성도 4건(test_command, REFLECTION, 토큰 예산, worktree)을 완료하면 **78~80점 도달** 예상.
P1 에이전트 팀 전환까지 완료하면 **80+ 도달** 예상.
