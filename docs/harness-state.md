...
| `harness-memory.md` | 크로스 프로젝트 실패/성공 패턴 저장 (S5 반자동: FAIL 시 초안 생성 → 유저 승인) | harness/impl_helpers.sh (CONSTRAINTS 로드) |
...
| 플래그 | 생성 주체 | 소비 주체 | 의미 |
|---|---|---|---|
| `{p}_harness_active` | harness/executor.sh | harness/impl_{fast,std,deep}.sh | 하네스 실행 중 |
| `{p}_plan_validation_passed` | validator (Plan Validation PASS) | harness/impl_{fast,std,deep}.sh (engineer 루프 진입 체크) | impl 파일 검증 완료 |
| `{p}_impl_path` | harness/executor.sh | harness/impl_{fast,std,deep}.sh | 현재 impl 파일 경로 |
| `{p}_current_issue` | harness/executor.sh | harness/impl_{fast,std,deep}.sh, PostToolUse 훅 | 현재 처리 중 이슈 번호 |
| `{p}_test_engineer_passed` | test-engineer (TESTS_PASS) | harness/impl_{fast,std,deep}.sh | 테스트 통과 |
| `{p}_validator_b_passed` | validator Mode B (PASS) | harness/impl_{fast,std,deep}.sh | 코드 검증 통과 |
| `{p}_pr_reviewer_lgtm` | pr-reviewer (LGTM) | harness/impl_{fast,std,deep}.sh | PR 리뷰 승인 |
| `{p}_security_review_passed` | security-reviewer (SECURE) | harness/impl_{fast,std,deep}.sh | 보안 감사 통과 |
...
| `{p}_pr_body.txt` | harness/impl_{fast,std,deep}.sh (HARNESS_DONE) | 메인 Claude (PR 생성 시 활용) | PR 본문 자동 생성 |
| `{p}_memory_candidate.md` | harness/impl_{fast,std,deep}.sh (FAIL 시) | 메인 Claude (유저 승인 후 harness-memory.md에 기록) | 실패 패턴 초안 (S5) |
| `{p}-agent-calls.log` | harness/impl_{fast,std,deep}.sh | - | 에이전트 호출 로그 |
| `{p}_harness_kill` | 사용자 (`/harness-kill`) | harness/utils.sh `kill_check()` | 킬 스위치 — 다음 에이전트 호출 전 루프 중단 (S31) |
| `{p}_{agent}_cost.txt` | `harness/utils.sh` `_agent_call()` | `harness/impl_helpers.sh` `budget_check()` | 에이전트별 비용 (USD). result 이벤트 `total_cost_usd` 추출 (S32) |
...
| Ground truth 테스트 (LLM 독립) | `npx vitest run` in `harness/impl_{fast,std,deep}.sh` | 초기 |
| 에이전트 도구 경계 물리적 차단 | `agent-boundary.py` | 초기 |
| 보안 감사 게이트 | `security-reviewer` (OWASP+WebView) | 초기 |
| Smart Context (50KB 캡) | `build_smart_context()` in `harness/impl_{fast,std,deep}.sh` | 초기 |
...
| S2 | PR body 자동 생성 | `harness/impl_{fast,std,deep}.sh` HARNESS_DONE 후 `/tmp/{p}_pr_body.txt` | 2026-04-05 |
...
| S17 | ~~JSON Lease~~ → pre-evaluator 유지 | `harness/impl_{fast,std,deep}.sh` run_automated_checks() (has_changes/no_new_deps/file_unchanged). JSON Lease 제거 (RF1) | 2026-04-07 |
...
| S21 | 타임스탬프 로깅 (hlog) | `harness/impl_helpers.sh` `hlog()` 함수 — `[HH:MM:SS] [attempt=N]` 형식. 루프 시작/종료·에이전트 전후·vitest 전후 기록. `/tmp/${PREFIX}-harness-debug.log` | 2026-04-06 |
| S22 | 에이전트 timeout + exit 124 감지 | `harness/utils.sh` `_call_exit` 전파 + `return $_call_exit`. `harness/impl_{fast,std,deep}.sh` 모든 `_agent_call`에 `\|\| AGENT_EXIT=$?` + exit 124 hlog | 2026-04-06 |
...
| S31 | 킬 스위치 | `harness/utils.sh` `kill_check()` — `/tmp/{p}_harness_kill` 감지 시 즉시 `HARNESS_KILLED` 출력 후 종료. while 루프 상단 + 에이전트 호출 직전 전수 삽입. `/harness-kill` 커맨드 추가. `harness/executor.sh` EXIT trap에서 kill 파일 정리 | 2026-04-06 |
| S32 | 전체 루프 비용 상한 $10 | `harness/impl_helpers.sh` `budget_check()` — stream-json result 이벤트에서 `total_cost_usd` 추출 → `TOTAL_COST` 누적 → $10 초과 시 `HARNESS_BUDGET_EXCEEDED` 출력 후 종료. hlog에 에이전트별·누적 비용 기록 | 2026-04-06 |
...
| S39 | agent out_file 가드 | `harness/impl_helpers.sh` `check_agent_output()` — 5곳 전수 적용. `harness/utils.sh` out_file 사전 touch | 2026-04-07 |
...
graph TD
  ...
  Executor -->|impl + depth 감지| Loop[harness/impl_{fast,std,deep}.sh\n구현 루프]
