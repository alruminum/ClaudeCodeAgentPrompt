...
- **Second Reviewer**: pr-reviewer(Claude)와 동시에 외부 AI(Gemini/GPT)를 비동기 병렬 실행. `config.second_reviewer` (예: `"gemini"`)로 on/off. LGTM 시 findings → POLISH 항목 합산. CLI 미설치/에러 → 자동 스킵.
