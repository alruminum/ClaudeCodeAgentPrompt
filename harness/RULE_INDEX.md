...
| R02 | pr-reviewer — fast/std/deep 전체 깊이에서 실행 | pr-reviewer.*fast/std/deep | impl_fast.sh, impl_std.sh, impl_deep.sh | commit-strategy: pr-reviewer runs on all depths (code analysis) |
...
| R07 | fast CHANGES_REQUESTED — pr-reviewer가 CHANGES_REQUESTED 시 engineer 추가커밋 1회 | fast-pr-fix | impl_fast.sh | dryrun: fast CHANGES_REQUESTED — engineer fix adds [fast-pr-fix] commit |
