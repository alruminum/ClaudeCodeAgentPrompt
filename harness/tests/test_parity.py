...
class TestImplFileAbsoluteResolve(unittest.TestCase):
    """run_simple이 impl_file 상대경로를 절대경로로 resolve해 chdir(worktree) 이후에도 안정적으로 읽는지.

    회귀 재현 (run_20260419_130701 / run_20260419_150018): 이전에는 cwd가 worktree로
    chdir된 뒤 build_smart_context가 상대경로를 worktree 기준으로 해석해 impl을 못 읽고
    ctx=""가 되었다. v05 디렉토리가 아직 main에 없는 feature 브랜치 초기 상태에서 발생.
    """
    def test_relative_impl_file_resolved_to_absolute(self):
        from unittest.mock import patch
        from harness.config import HarnessConfig
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for cmd in (
                ...
                subprocess.run(cmd, capture_output=True, cwd=td)
            ...
            def _capture_bsc(impl, attempt_n, err_trace=""):
                captured_paths.append(impl)
                # 실제 읽기는 불필요 — 경로만 확인
                return "mock ctx"

            def mock_agent_call(agent, timeout, prompt, out_file, *args, **kwargs):
                Path(out_file).write_text(f"mock {agent}\n---MARKER:LGTM---\n")
                return 0
            ...
            with patch("harness.impl_loop.agent_call", side_effect=mock_agent_call), \
                 ...
                 patch("harness.impl_loop.collect_changed_files", return_value=["src/test.ts"]), \
                 patch("harness.impl_loop.run_automated_checks", return_value=(True, "")), \
                 patch("harness.impl_loop.build_smart_context", side_effect=_capture_bsc), \
                 patch("subprocess.run") as mock_sub:
                mock_sub.return_value = subprocess.CompletedProcess(args=[], returncode=0, stdout="abc123", stderr="")
                ...
                rl = RunLogger("test", "impl", "1")
                run_simple(impl_rel, "1", config, sd, "test", "feat", rl)

            self.assertTrue(captured_paths, "build_smart_context was never called")
            for p in captured_paths:
                self.assertTrue(
                    os.path.isabs(str(p)),
                    f"impl_file 인자가 절대경로로 resolve되지 않음: {p}",
                )
