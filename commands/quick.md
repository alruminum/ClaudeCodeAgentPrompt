---
name: quick
description: 작은 버그픽스·마이너 수정을 한 줄로 받아 하네스 루프 전체를 자동 진입시키는 스킬. 유저가 "간단히 해줘", "작은 수정이야", "한 줄 버그", "/quick", "퀵", "바로 고쳐줘" 등을 말할 때 반드시 이 스킬을 사용한다. 내부적으로 qa → architect Mode F → executor.py impl (simple) 을 체이닝한다. 루프 불변식(plan validation, LGTM, src 경계)은 그대로 유지.
---
...
@MODE:QA:ANALYZE
@PARAMS: {
  "issue": "[유저 원문] <원문 그대로>\n[Quick 모드] depth=simple 고정, architect Mode F 진입 예정",
  "quick_mode": true
}
```
...
executor.py 가 내부적으로:
- plan 루프 → architect Mode F (LIGHT_PLAN) — 이슈 번호 전달
