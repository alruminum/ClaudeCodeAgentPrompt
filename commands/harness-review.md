...
**리포트 원문을 한 글자도 바꾸지 말고 그대로 출력하라.**

금지 행위:
- 테이블을 다른 형식(박스, 리스트, 요약표)으로 재가공
- 섹션 생략 또는 축약
- "핵심 원인은~" 같은 자체 해석을 리포트 사이에 삽입
- 마크다운 테이블을 ASCII 박스로 변환

허용 행위:
- 리포트 전문 출력 **후** 별도 줄에서 추가 코멘트 작성

리포트 텍스트를 `print()` 또는 `cat`한 결과가 유저에게 보이는 최종 출력이다. Claude가 중간에 개입하지 마라.
