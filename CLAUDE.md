...
**새 에이전트를 작성하거나 기존 에이전트를 수정할 때** 반드시 아래 규칙을 따른다.
프로젝트 에이전트 파일(`.claude/agents/*.md`) 맨 앞에서 해당 base를 Read한 뒤, 프로젝트 특화 지침만 추가한다.
base에 없는 내용만 프로젝트 에이전트에 추가한다 — base와 중복되는 규칙은 넣지 않는다.

```markdown
## Base 지침 (항상 먼저 읽기)

작업 시작 전 `~/.claude/agents/[base-name].md`를 Read 툴로 읽고 그 지침을 모두 따른다.
아래는 이 프로젝트에만 적용되는 추가 지침이다.
```

| 에이전트 유형 | Base 파일 |
|---|---|
| 기획자 에이전트 | `~/.claude/agents/product-planner-base.md` |
| 오케스트레이터 | `~/.claude/agents/orchestrator-base.md` |
| 코드 구현 에이전트 | `~/.claude/agents/engineer-base.md` |
| 검증 에이전트 | `~/.claude/agents/validator-base.md` |
| UI 디자인 생성 에이전트 | `~/.claude/agents/designer-base.md` |
| 디자인 심사 에이전트 | `~/.claude/agents/design-critic-base.md` |
| 설계/계획 에이전트 | `~/.claude/agents/architect-base.md` |
