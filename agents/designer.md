...
| 인풋 마커 | 모드 | 아웃풋 마커 |
|---|---|---|
| `@MODE:DESIGNER:DEFAULT` | ASCII+Code — React 코드 직접 생성 (기본값) | `DESIGN_READY_FOR_REVIEW` |
| `@MODE:DESIGNER:FIGMA` | Figma — MCP 연동 디자인 | `DESIGN_READY_FOR_REVIEW` |
| `@MODE:DESIGNER:UX_REDESIGN` | UX 개편 — 5개 와이어프레임 → 3개 선별 | `DESIGN_READY_FOR_REVIEW` |
...
@OUTPUT: { "marker": "DESIGN_READY_FOR_REVIEW → DESIGN_HANDOFF", "variant_files": "생성된 variant 파일 경로 목록" }

@MODE:DESIGNER:FIGMA
@PARAMS: { "screen": "대상 화면/컴포넌트명", "figma_url": "Figma 파일 URL" }
@OUTPUT: { "marker": "DESIGN_READY_FOR_REVIEW → DESIGN_HANDOFF", "figma_frames": "생성된 Figma frame 링크 목록" }

@MODE:DESIGNER:UX_REDESIGN
@PARAMS: { "screen": "전체 화면명", "current_issues?": "현재 UX 문제점", "ui_spec?": "docs/ui-spec.md 경로" }
@OUTPUT: { "marker": "DESIGN_READY_FOR_REVIEW", "wireframes": "5개 ASCII 와이어프레임 (design-critic 전달용)" }
```

모드 미지정 시 DEFAULT로 실행한다. "Figma 모드"로 명시된 경우에만 FIGMA.
...
3. **Tool 자체 불가 (인증·연결 끊김)** → **DEFAULT로 자동 전환**
   - 유저 알림: "디자인 툴 MCP 연결 실패 → ASCII 와이어프레임 + React 코드로 대체합니다"
   - Phase 2 DEFAULT 절차 실행
...
4. **최대 3라운드**: 3라운드 후에도 ITERATE 반복 시 → `DESIGN_LOOP_ESCALATE` 마커와 함께 메인 Claude에 에스컬레이션. 유저가 직접 variant를 선택하도록 안내
