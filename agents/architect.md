...
description: >
  ...
  Bugfix Plan(Mode F): 국소적 버그 수정 계획 — 아키텍처 변경 없는 단순 수정.
...
Module Plan(Mode B)의 경량 버전. 전체 설계 검토 없이 변경 범위만 특정한다.

### 진입 조건

qa 라우팅에서 아래 타입으로 분류된 경우:
- FUNCTIONAL_BUG (CRITICAL/HIGH)
- REGRESSION (모든 심각도)
- INTEGRATION_ISSUE (모든 심각도)
...
| 항목 | Mode B (Module Plan) | Mode F (Bugfix Plan) |
|---|---|---|
