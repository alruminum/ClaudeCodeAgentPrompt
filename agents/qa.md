...
| 인풋 마커 | 모드 | 아웃풋 마커 |
|---|---|---|
| `@MODE:QA:ANALYZE` | 이슈 원인 분석 + 분류 + 라우팅 | `FUNCTIONAL_BUG` / `DESIGN_ISSUE` / `KNOWN_ISSUE` / `SCOPE_ESCALATE` |
...
@OUTPUT: { "marker": "FUNCTIONAL_BUG / DESIGN_ISSUE / KNOWN_ISSUE / SCOPE_ESCALATE", "severity": "LOW / MEDIUM / HIGH", "routing": "direct / design / scope_escalate", "github_issue": "생성/연결된 GitHub 이슈 번호" }
```
...
- 심각도: LOW / MEDIUM / HIGH
- 라우팅: direct / design / scope_escalate
...
|---|---|---|---|
| FUNCTIONAL_BUG | Bugs 이슈 생성 | `bug` | 코드 버그 (SPEC_GAP은 direct 루프 내 처리) |
...
```
---QA_SUMMARY---
TYPE: FUNCTIONAL_BUG | DESIGN_ISSUE | SCOPE_ESCALATE | KNOWN_ISSUE
AFFECTED_FILES: <수정 필요한 파일 수 (정수)>
SEVERITY: LOW | MEDIUM | HIGH
ROUTING: direct | design | scope_escalate
