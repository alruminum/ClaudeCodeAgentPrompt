...
description: >
  설계와 코드를 검증하는 에이전트.
  Mode A: 시스템 설계 검증 (architect SYSTEM_DESIGN_READY → 구현 가능성 검증) — Orchestrator Phase 1에서 호출.
  Mode B: 코드 검증 (구현 완료 코드 → 스펙·의존성·품질 검증) — Orchestrator Phase 3에서 호출.
...
저장 주체: orchestrator (validator는 Write 도구 없음)
...
- **Mode B 재검증 최대 3회**: 3회 초과 시 `VALIDATION_ESCALATE` 마커와 함께 orchestrator에 에스컬레이션
