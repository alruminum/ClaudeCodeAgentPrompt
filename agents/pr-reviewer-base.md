---
name: pr-reviewer-base
description: >
  validator PASS 이후, git commit 전에 코드 품질을 리뷰하는 에이전트 base.
...
**권장 모델: `opus`** — 코드 품질 판단은 맥락 이해와 시니어 시각이 핵심
...
4. **최대 3라운드**: 3라운드 후에도 MUST FIX가 남으면 `REVIEW_LOOP_ESCALATE` 마커와 미해결 항목 목록을 orchestrator에 전달하고 중단
...
- **루프 한도**: CHANGES_REQUESTED 재검토 최대 3라운드. 초과 시 orchestrator 에스컬레이션
